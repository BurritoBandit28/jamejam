...
void StringToLevelConverter::convert() {
    int depth = 0;
    bool atValue = false;
    bool atString = false;
...
TileLayer StringToLevelConverter::readTileLayer(std::string name, int index) {
    int depth = 0;
    int pointInTile = 0;
    int i = index + 1;
    bool atValue = false;
    ...
    return TileLayer(name, resolution, gridx, gridy, parallaxModifier, tiles);
}
